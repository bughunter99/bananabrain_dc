...
//If your plugin need additional files use the PluginPath you get passed in LoadInfo/InjectedInfo to locate them. Don't assume the plugin is in the same directory as the launcher or the ame. 
...
type TDisplayLocalTextMessageFunc=procedure(Msg:PChar);stdcall; 
     ...
     TPluginCompatibilityCheckCallback=procedure(Compatibility:integer;const Info:TPluginCompatibilityCheckInfo;const PluginInfo:TPluginInfo;UserData:pointer);stdcall; 
...
type TPluginCompatibilityCheckInfo=packed record 
  ...
  GameVersion:TVersion;//Version of the game 
end; 
  
type TGeneralInfo=packed record//All readonly
  StructSize:Cardinal;//sizeof(TGeneralInfoo) 
  LauncherApiMajor:Word;//Major version of LauncherAPI, Must be equal to expected version 
  LauncherApiMinor:Word;//Minor version of LauncherAPI, Must more or equal than the expected version 
  GameID:PChar;//ID of the game, currently always 'Starcraft'
  ...
  GamePath:PChar;//Absolute Path to the game 
end;

type TLoadPluginInfo=packed record//All readonly
  StructSize:Cardinal;//sizeof(TLoadPluginInfo)
end;

type TLoadExtensionInfo=packed record//All readonly
  StructSize:Cardinal;//sizeof(TLoadExtensionInfo)
...
type TRunInLauncherInfo=packed record
  StructSize:Cardinal;//sizeof(TRunInLauncherInfo) 
end;
...
Type TPluginInfo=packed record 
  ...
  Author:PChar;//RW, 256 bytes Memory, Auto prefilled from Versioninfo-Ressource("CompanyName") 
  Description:PChar;//RW, 64k bytes Memory, Auto prefilled from Versioninfo-Ressource("FileDescription") 
  UpdateUrl:PChar;//RW, 1024 bytes Memory 
  PublicKey:PChar;//RW, 2048 bytes Memory 
  BanRisk:integer;//RW, 0=none(no interaction with SC whatsoever),1=low(no injection),2=medium(injection but no hooks or hooks outside Game-Code),3=high(hooks),-1 Unknown 
  ...
  NonHooking:BOOL;//RW, Hooks no the game functions, be carefull in conjunction with RegisterCallback, as that can add hooks too 
...
function Chl1_Load(const GeneralInfo:TGeneralInfo;

//Called when the launcher loads the plugin in the process of the launcher 
function Chl1_LoadPlugin(const GeneralInfo:TLoadInfo;const LoadPluginInfo:TLoadPluginInfo;var PluginInfo:TPluginInfo):BOOL;stdcall; 
begin 
  result:=false; 
  //Check API versions 
  if (LoadInfo.LauncherApiMajor<>WANTED_MAJOR)or(LoadInfo.LauncherApiMinor<WANTED_MINOR)then exit;//failure=>return false 
  if LoadInfo.GameID<>'Starcraft' then exit;//failure=>return false 
  //Fill PluginInfo 
  if (PluginInfo.StructSize<sizeof(TPluginInfo))then exit;//failure=>return false; 
  StrPLCopy(PluginInfo.PluginName,'SamplePlugin',256); 
  ...
  PluginInfo.NonHooking:=true;//Hooks no the game functions, be carefull in conjunction with RegisterCallback, as that can add hooks too 
...
function Chl1_PluginInject(const InjectedInfo:TInjectedInfo;var InjectedPluginInfo:TInjectedPluginInfo):BOOL;stdcall; 
begin 
  //Called after loading the Plugin in the game process 
  //InjectedInfo.IsLataActivation indicates if the game is still suspended 
...
function Chl1_PluginRun(const RunInfo:TRunInfo):BOOL;stdcall; 
begin 
...
procedure Chl1_PluginShowConfig();stdcall; 
begin 
  //show config dialog here 
end; 

procedure Chl1_PluginIsCompatible(var Compatibility:integer;const GameVersion:TVersion;Ladder:PChar);stdcall; 
begin 
...
exports Chl1_PluginLoad,Chl1_PluginInject,Chl1_PluginRun,Chl1_PluginShowConfig,Chl1_PluginIsCompatible;