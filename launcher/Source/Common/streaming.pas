...
Type TPointerStream=class(TCustomMemoryStreamEx)
  public
    procedure SetAccess(ACanRead,ACanWrite:Boolean);
    procedure SetPointer(Ptr: Pointer; Size: {$IFDEF CPU64}Int64{$ELSE}Longint{$ENDIF});
end;
...
begin
  if not CanWrite then raise EStreamAccess.create('Count not write to readonly stream');
  if count>Size-Position then count:=Size-Position;
  move(Buffer,Memory^,Count);
  result:=count;
end;
...
procedure TPointerStream.SetPointer(Ptr: Pointer; Size: {$IFDEF CPU64}Int64{$ELSE}Longint{$ENDIF});
begin
  inherited;
end;
...
function TStreamHelper.ReadStringZT: String;
var c:char;
begin
  result:='';
  ReadBuffer(c,1);
