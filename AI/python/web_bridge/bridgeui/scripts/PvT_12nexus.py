"""PvT 12 Nexus (C++ opening_PvT_12Nexus 기반)

빌드 오더:
  supply 8  → Pylon ×1, scout (맵 중앙으로)
  supply 12 → 앞마당 Nexus 확장
  supply 13 → Gateway ×1, Assimilator ×1
  supply 15 → Cybernetics Core ×1
  Cybernetics Core 완성 → C++ Main 위임
"""
import sys, os; sys.path.insert(0, os.path.dirname(__file__))
from _helpers import StrategyHelper

CPP_OPENING = "PvT_12nexus"
...
def run(ctx):
    ...
    if not h.setup():
        return
    ctx.log("PvT 12 Nexus 오프닝 시작 (supply milestone)")
    h.start_trace("PvT_12Nexus", interval=1.5)
    scout_sent = False
    natural_sent = False
    ...
    while not ctx._stopped:
        ...
        if h.enemy_offense_larger_than_defense(cushion=1) or h.opening_lost_too_many_workers(margin=3):
            h.mark_once("fallback_main", "12 Nexus 위기 감지 → C++ Main으로 즉시 전환")
