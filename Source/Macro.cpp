...
void BuildingManager::apply_upgrades(bool important)
{
	for (auto& information_unit : information_manager.my_units()) {
		...
		if (unit->isPowered() && unit->isIdle()) {
			// Skip if a command was recently issued to this unit that we didn't initiate
			// (e.g. user manually clicked upgrade)
			auto last_cmd = unit->getLastCommand();
			bool recently_commanded = (Broodwar->getFrameCount() - unit->getLastCommandFrame()
			                           <= Broodwar->getLatencyFrames());
			if (recently_commanded &&
				(last_cmd.getType() == UnitCommandTypes::Upgrade ||
				 last_cmd.getType() == UnitCommandTypes::Research)) {
				continue;
			}
...
void TrainingManager::apply_train_orders(TrainDistribution& train_queue,MineralGas& reserved)
{
	...
	for (auto& information_unit : information_manager.my_units()) {
		...
		if (building->isCompleted() && building->isPowered() && building->getType() == train_queue.builder_type()) {
			// Skip if player recently issued a train command the bot didn't initiate
			auto last_cmd = building->getLastCommand();
			bool recently_player_trained = (last_cmd.getType() == UnitCommandTypes::Train &&
			                                Broodwar->getFrameCount() - building->getLastCommandFrame()
			                                <= Broodwar->getLatencyFrames() &&
			                                !building->getTrainingQueue().empty());
			if (recently_player_trained) continue;
